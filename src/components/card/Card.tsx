...
function Card({ colab, modoLista=false }: CardProps) {
    return (
        <div className={`bg-[#0a2e2e]/60 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-6 shadow-2xl ${modoLista ? 'flex flex-row items-center gap-8' : 'flex flex-col gap-6'}`}>
            ...
            <div className={`flex ${modoLista ? 'flex-row items-center gap-8 flex-1' : 'flex-col gap-6'}`}>
                ...
                <div className={`flex ${modoLista ? 'flex-row gap-8' : 'flex-col gap-2 pt-2 border-t border-white/5'}`}>
